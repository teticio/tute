  ...
  if (absl::Uniform(rng_, 0.0, 1.0) < epsilon) {
    ...
    torch::Tensor info_state_tensor =
        torch::from_blob(info_state.data(), {info_state.size()},
                         torch::dtype(torch::kFloat32)).to(device_)
            .view({1, -1});
    q_network_->eval();
    torch::Tensor q_values = q_network_->forward(info_state_tensor).detach();
    torch::Tensor illegal_actions_mask =
        torch::full({num_actions_}, true, torch::dtype(torch::kBool)).to(device_);
...
void DQN::Learn() {
  ...
  for (auto t : transition) {
    info_states.push_back(
        torch::from_blob(
            t.info_state.data(),
            {1, t.info_state.size()},
            torch::dtype(torch::kFloat32)).clone().to(device_));
    ...
    legal_actions_mask.push_back(
        torch::from_blob(t.legal_actions_mask.data(),
                         ...
                         torch::dtype(torch::kInt32)).to(device_)
            .clone());
  ...
  torch::Tensor illegal_action_masks_tensor =
      (1.0 - torch::stack(legal_actions_mask, 0)).to(torch::dtype(torch::kBool));
  ...
  torch::Tensor are_final_steps_tensor = torch::from_blob(
      ...
      {batch_size_},
      torch::dtype(torch::kInt32)).to(torch::dtype(torch::kFloat32).device(device_));
  ...
  torch::Tensor actions_tensor = torch::from_blob(
      actions.data(),
      {batch_size_},
      torch::dtype(torch::kInt64)).to(device_);
  torch::Tensor predictions = q_values.index(
      {torch::arange(q_values.size(0)).to(device_),
